...
const Funds = () => {
  ...
  const fetchBalance = async () => {
    try {
      const res = await axios.get(`https://stocktrading-c41p.onrender.com/balance/${userId}`);
      // backend returns number or object; handle both
  ...
  const handleAddFunds = async () => {
    try {
      const res = await axios.post(`https://stocktrading-c41p.onrender.com/addBalance/${userId}`);
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const res = await axios.patch(`https://stocktrading-c41p.onrender.com/withdraw/${userId}`, {
        amount: amt,
      });
  ...
  return (
    <div className="funds-root">
      ...
      {/* Example summary table (non-blocking) */}
      ...
      {/* Withdraw modal - implemented as centered element, not a full-screen blocking overlay */}
      {showWithdrawModal && (
        <div className="modal-backdrop" onClick={closeWithdraw} role="presentation">
          <div
            className="modal"
            onClick={(e) => e.stopPropagation()} /* prevent backdrop click from closing when interacting */
          >
            <h4>Withdraw</h4>
            <input
              type="number"
