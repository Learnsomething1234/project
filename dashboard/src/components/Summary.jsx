...
import "./summary.css";
import { ArrowUpward, ArrowDownward } from "@mui/icons-material"; // MUI icons
...
const Summary = () => {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        // Fetch User
        const userRes = await axios.get(`https://stocktrading-c41p.onrender.com/user/${id}`);
        setUser(userRes.data);

        // Fetch Balance
        const balanceRes = await axios.get(`https://stocktrading-c41p.onrender.com/balance/${id}`);
        setBalance(balanceRes.data);

        // Fetch Holdings
        const holdingsRes = await axios.get(`https://stocktrading-c41p.onrender.com/holdings/${id}`);
